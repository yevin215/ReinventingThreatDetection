...
struct ContentView: View {
    ...
    var body: some View {
        VStack(spacing: 12) {
            ...
            if let metrics = sdk.metricsBuffer {
                ...
                .font(.headline)
                .onAppear {
                    sendMetrics(heartRate: Double(pulse), breathingRate: Double(breath))
                }
            } else {
    ...
    func sendMetrics(heartRate: Double, breathingRate: Double) {
        ...
        URLSession.shared.dataTask(with: request) { _, _, error in
            if let error = error { print("❌ Send failed:", error.localizedDescription) }
            else { print("✅ Sent — HR: \(heartRate), BR: \(breathingRate)") }
        }.resume()
    }
}